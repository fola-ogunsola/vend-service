...
const validateData = (schema, type) => async(req, res, next) => {
  try {
    ...
    if (!isValid.error) {
      logger.info(`${enums.CURRENT_TIME_STAMP}, Info: successfully validates request parameters validateData.user.middlewares.model.js`);
      return next();
    }
    logger.info(`${enums.CURRENT_TIME_STAMP}, Info: failed to validate request parameters validateData.user.middlewares.model.js`);
    const { message } = isValid.error.details[0];
    return Response.error(res, message.replace(/["]/gi, ''), enums.HTTP_UNPROCESSABLE_ENTITY, enums.VALIDATE_DATA_MIDDLEWARE);
  } catch (error) {
